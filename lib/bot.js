...
class Bot {
  constructor(options) {
	...
	this.rIgnoreUsers = options.rIgnoreUsers || {};
	this.rIgnoreUsers.irc = this.rIgnoreUsers.irc || [];
	this.rIgnoreUsers.discord = this.rIgnoreUsers.discord || [];
  ...
  rIgnoredIrcUser(user) {
	return this.rIgnoreUsers.irc.some(i => new RegExp(i).test(user));
  }
  ...
  rIgnoredDiscordUser(user) {
	return this.rIgnoreUsers.discord.some(i => new RegExp(i).test(user));
  }
  ...
  sendToIRC(message) {
    ...
    if (this.ignoredDiscordUser(author.username) || this.rIgnoredDiscordUser(author.username)) {
      return;
    }
  ...
  sendToDiscord(author, channel, text) {
    ...
    if (this.ignoredIrcUser(author) || this.rIgnoredIrcUser(author)) {
      return;
    }
